...
pub struct FileSnippetData {
    ...
    /// the `lnum`, and then hostname and content. Ideally we would want it to just compare `lnum`.
    /// So why don't you just manually implement the traits then? Because, [`PartialOrd`] uses
    /// the [`PartialEq`] implementation and so we would have to make [`PartialEq`] only
